...
#include <parmexpr/hana/detail/preprocessor.hpp>
...
#   define BOOST_HANA_RUNTIME_CHECK_MSG(condition, message)                 \
    do {                                                                    \
        ...
        static_assert(!::parmexpr::hana::Constant<decltype(__hana_tmp)>::value,\
...
#   define BOOST_HANA_CONSTANT_CHECK_MSG(condition, message)                \
        ...
        static_assert(::parmexpr::hana::Constant<                              \
        ...
        static_assert(::parmexpr::hana::value<                                 \
            decltype(BOOST_HANA_PP_CONCAT(__hana_tmp_, __LINE__))           \
        >(), message);                                                      \
        static_assert(true, "force trailing semicolon")                     \
/**/
...
#   define BOOST_HANA_CHECK_MSG(condition, message)                         \
    do {                                                                    \
        auto __hana_tmp = condition;                                        \
        ::parmexpr::hana::if_(::parmexpr::hana::bool_c<                           \
            ::parmexpr::hana::Constant<decltype(__hana_tmp)>::value>,          \
            [](auto expr) {                                                 \
                static_assert(::parmexpr::hana::value<decltype(expr)>(),       \
                message);                                                   \
            },                                                              \
            [](auto expr) {                                                 \
